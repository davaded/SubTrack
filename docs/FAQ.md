# 🙋 常见问题解答 (FAQ)

## 部署相关

### Q: 镜像地址是什么？我需要设置什么环境变量吗？

**A:** 不需要设置任何环境变量！

**简短回答：** 镜像地址已经在 `docker-compose.prod.yml` 中配置好了。

**详细解释：**

SubTrack 提供两种部署方式：

#### 方式 1：使用预构建镜像（推荐）⭐

```bash
docker-compose -f docker-compose.prod.yml pull
docker-compose -f docker-compose.prod.yml up -d
```

**镜像地址：** `ghcr.io/davaded/subtrack:latest`
- Docker 镜像已经预先构建好，发布在 GitHub Container Registry
- 这是一个**公开镜像**，所有人都可以免费使用
- 就像你从 Docker Hub 拉取 `nginx:latest` 或 `postgres:15` 一样

**优点：**
- ✅ 部署快速（1-2 分钟）
- ✅ 不需要自己构建
- ✅ 已经过测试和优化
- ✅ 无需设置环境变量

**类比：**
这就像你安装 Nginx 时使用 `docker pull nginx:latest`，你也不是 Nginx 的作者，但你使用他们官方发布的镜像。SubTrack 也是一样的道理。

#### 方式 2：自己构建镜像

如果你想完全自主控制，可以自己构建：

```bash
# 不使用预构建镜像，在本地构建
docker-compose up -d --build
```

**优点：**
- ✅ 完全自主，不依赖他人
- ✅ 可以自定义修改

**缺点：**
- ❌ 构建时间长（5-10 分钟）
- ❌ 需要服务器资源（CPU、内存）

---

### Q: 如果我 fork 了项目，配置了自己的 GitHub Actions，应该怎么做？

**A:** 如果你想发布自己的镜像：

1. **Fork 项目到你的 GitHub 账号**
2. **GitHub Actions 会自动发布到你的 GHCR**
3. **修改 `docker-compose.prod.yml`：**

```yaml
services:
  web:
    image: ghcr.io/你的用户名/subtrack:latest
```

4. **然后部署：**

```bash
docker-compose -f docker-compose.prod.yml pull
docker-compose -f docker-compose.prod.yml up -d
```

---

### Q: 预构建镜像安全吗？我能信任吗？

**A:** 完全透明和可验证：

1. **源代码公开**
   - 所有代码都在 GitHub 上：https://github.com/davaded/SubTrack
   - 可以审查所有文件

2. **构建过程公开**
   - GitHub Actions 构建日志完全公开
   - 访问：https://github.com/davaded/SubTrack/actions
   - 可以查看每一步的构建过程

3. **镜像可验证**
   - 每个镜像都有对应的 Git commit SHA
   - 可以追溯到具体的代码版本

4. **如果还是不放心**
   - 使用方式 2：自己构建镜像
   - Fork 项目，自己维护

---

### Q: 我能把项目改名叫 MySubTrack 并发布自己的镜像吗？

**A:** 可以！这是开源项目的优势：

1. **Fork 项目**
2. **修改项目名称和配置**
3. **推送到你的仓库**
4. **GitHub Actions 会自动发布到你的 GHCR**
5. **使用你自己的镜像部署**

这完全符合 MIT 开源协议。

---

## 功能相关

### Q: 必须配置邮件提醒吗？

**A:** 不是必须的。

- 邮件提醒是**可选功能**
- 不配置也能正常使用所有核心功能
- 如果不需要邮件提醒，`.env` 中的以下配置可以留空：
  ```bash
  RESEND_API_KEY=""
  EMAIL_FROM=""
  ```

---

### Q: 支持哪些通知渠道？

**A:** 目前支持三种，可以单独使用或组合使用：

1. **邮件通知** - 使用 Resend 服务
2. **钉钉通知** - 企业群机器人
3. **飞书通知** - 企业群机器人

全部是可选的，根据需要配置。

---

### Q: 默认管理员账号安全吗？

**A:** 首次部署时会创建默认管理员账号，但是：

1. **密码在 `.env` 文件中配置** - 你自己设置
2. **首次登录后会提示修改密码** - `mustChangePassword: true`
3. **建议立即修改默认密码** - 这是安全最佳实践

---

### Q: 注册模式有什么区别？

**A:** 三种模式适用于不同场景：

| 模式 | 适用场景 | 说明 |
|------|---------|------|
| **Open（开放）** | 个人使用、小团队 | 任何人都可以注册 |
| **Approval（审批）** | 小公司、部门内部 | 新用户需要管理员批准 |
| **Closed（关闭）** | 私人使用 | 禁止注册，只有管理员创建账号 |

可以在 `/admin/settings` 随时修改。

---

## 技术相关

### Q: 为什么使用 PostgreSQL 而不是 MySQL？

**A:** PostgreSQL 的优势：

- ✅ 更强大的 JSON 支持
- ✅ 更好的数据完整性
- ✅ Prisma ORM 对 PostgreSQL 支持最好
- ✅ 开源且免费

当然，如果需要，可以修改 Prisma schema 来支持 MySQL。

---

### Q: 可以部署到 Vercel 吗？

**A:** 理论上可以，但有限制：

**适合 Vercel：**
- ✅ 前端部分
- ✅ API Routes

**不适合 Vercel：**
- ❌ 需要自己的 PostgreSQL 数据库
- ❌ Vercel Postgres 有免费额度限制
- ❌ Docker 部署更简单完整

**推荐：** Docker 部署 > Vercel 部署

---

### Q: 支持多用户吗？

**A:** 完全支持！

- ✅ 每个用户独立的订阅数据
- ✅ 用户之间数据隔离
- ✅ 管理员可以管理所有用户
- ✅ 支持用户审批机制

---

### Q: 数据可以导出吗？

**A:** 可以通过数据库备份导出：

```bash
# 导出整个数据库
docker-compose -f docker-compose.prod.yml exec db pg_dump -U postgres subtrack > backup.sql

# 导出特定表
docker-compose -f docker-compose.prod.yml exec db pg_dump -U postgres -t subscriptions subtrack > subscriptions.sql
```

未来版本会添加 CSV/Excel 导出功能。

---

### Q: 忘记管理员密码怎么办？

**A:** 重置方法：

**方法 1：重置整个数据库（会丢失所有数据）**
```bash
docker-compose -f docker-compose.prod.yml down
docker volume rm subtrack_postgres_data
docker-compose -f docker-compose.prod.yml up -d
```

**方法 2：直接修改数据库（保留数据）**
```bash
# 1. 进入数据库容器
docker-compose -f docker-compose.prod.yml exec db psql -U postgres subtrack

# 2. 查看用户
SELECT id, email, role FROM users;

# 3. 重置密码（设置 mustChangePassword）
UPDATE users SET "must_change_password" = true WHERE role = 'admin';

# 4. 或者直接重置为已知密码（需要先生成 hash）
# 在本地运行 Node.js：
# const bcrypt = require('bcrypt');
# console.log(await bcrypt.hash('newpassword', 10));
```

---

### Q: 如何升级到新版本？

**A:** 非常简单：

```bash
cd ~/SubTrack
git pull origin main
export GITHUB_USERNAME=davaded
docker-compose -f docker-compose.prod.yml pull
docker-compose -f docker-compose.prod.yml up -d
```

数据库迁移会自动执行。

---

### Q: 性能如何？能支持多少用户？

**A:** 实测数据：

**配置：** 1核 1GB RAM

- ✅ 支持 10-50 个并发用户
- ✅ 响应时间 < 200ms
- ✅ 数据库查询优化（索引）

**配置：** 2核 2GB RAM

- ✅ 支持 100+ 并发用户
- ✅ 可以处理数万条订阅记录

---

## 故障排查

### Q: 端口 3000 被占用怎么办？

**A:** 修改端口：

1. **编辑 `docker-compose.prod.yml`：**
```yaml
services:
  web:
    ports:
      - "8080:3000"  # 改成 8080 或其他端口
```

2. **重启服务：**
```bash
docker-compose -f docker-compose.prod.yml up -d
```

3. **访问：** `http://你的IP:8080`

---

### Q: 容器启动失败，日志显示数据库连接错误？

**A:** 常见原因：

1. **数据库容器未启动**
```bash
docker-compose -f docker-compose.prod.yml ps
# 确保 db 容器状态是 Up
```

2. **数据库密码配置错误**
```bash
# 检查 .env 文件中的 POSTGRES_PASSWORD
# 确保与 docker-compose.prod.yml 中的配置一致
```

3. **等待数据库启动**
```bash
# 数据库需要 10-20 秒启动，web 容器可能启动太快
docker-compose -f docker-compose.prod.yml restart web
```

---

### Q: 如何查看详细错误日志？

**A:** 多种方式：

```bash
# 查看所有日志
docker-compose -f docker-compose.prod.yml logs

# 只看 web 容器日志
docker-compose -f docker-compose.prod.yml logs web

# 实时查看日志
docker-compose -f docker-compose.prod.yml logs -f

# 查看最后 100 行
docker-compose -f docker-compose.prod.yml logs --tail=100
```

---

## 更多问题？

- 📖 **查看文档**：
  - [快速开始](../QUICKSTART.md)
  - [Docker 部署](../GITHUB_DEPLOY.md)
  - [管理员指南](../ADMIN_GUIDE.md)

- 🐛 **报告问题**：
  - https://github.com/davaded/SubTrack/issues

- 💬 **讨论交流**：
  - https://github.com/davaded/SubTrack/discussions
